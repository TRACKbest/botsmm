            ...
            async def handle_new_message(event):
                while True:
                    ...
                    if await self.process_instagram_account(client, instagram_account):
                        logging.info(f"Tâche terminée pour {instagram_account}")
                    else:
                        logging.info(f"Passage au compte Instagram suivant")
    ...
    def parse_task_message(self, message: str) -> Dict:
        ...
        try:
            ...
            if 'follow' in message.lower():
                ...
                link_match = re.search(r'(https?://[^\s]+)', message)
                task['link'] = link_match.group(0) if link_match else ''
                comment_match = re.search(r'with text: (.*?)(?:\n|$)', message)
            ...
            logging.error(f"Erreur lors de l'analyse du message: {e}")
            return None
