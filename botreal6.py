...
os.makedirs('accountstelegram', exist_ok=True)
os.makedirs('accountsinstagram', exist_ok=True)
...
class HumanBehavior
    @staticmethod
    def get_random_delay(min_seconds float = 2.0, max_seconds float = 6.0) - float
        Retourne un délai aléatoire pour simuler un comportement humain lent
        return random.uniform(min_seconds, max_seconds)

    @staticmethod
    def get_typing_delay(text str) - float
        Simule le temps de frappe humain, plus lent
        words = len(text.split())
        return (words  30)  60 + random.uniform(1.0, 4.0)

    @staticmethod
    def get_scroll_delay() - float
        Simule le temps de défilement humain, plus lent
        return random.uniform(2.0, 5.0)

    @staticmethod
    def get_reading_delay(text str) - float
        Simule le temps de lecture humain, plus lent
        words = len(text.split())
        return (words  150)  60 + random.uniform(2.0, 6.0)

class ConfigManager
    def __init__(self)
        self.config_file = 'configapi_config.json'
        self.load_config()
    ...
    def load_config(self)
        ...
        try
            ...
            logging.error(fErreur lors du chargement de la configuration {e})
            self.config = {}
    ...
    def save_config(self)
        ...
        try
            ...
            logging.error(fErreur lors de la sauvegarde de la configuration {e})

    def get_api_credentials(self)
        Demande et stocke les credentials API
        print(fn{Fore.CYAN}=== Configuration des identifiants API Telegram ==={Style.RESET_ALL})
        ...
        if 'api_id' not in self.config or 'api_hash' not in self.config
            ...
            print(fAPI ID  {self.config['api_id']})
            print(fAPI Hash  {self.config['api_hash'][8]}... (caché pour sécurité))
            
            change = input(nVoulez-vous modifier ces identifiants  (on)  ).lower()
            if change == 'o'
...
class AccountManager
    def __init__(self)
        self.accounts_file = 'configtelegram_accounts.json'
        self.load_accounts()
    ...
    def load_accounts(self)
        ...
        try
            ...
            logging.error(fErreur lors du chargement des comptes {e})
            self.accounts = {}
    ...
    def save_accounts(self)
        ...
        try
            ...
            logging.error(fErreur lors de la sauvegarde des comptes {e})
    ...
    async def validate_telegram_account(self, phone str, api_id str, api_hash str, session_name str) - bool
        ...
        try
            ...
            if not await client.is_user_authorized()
                ...
                code = input(fEntrez le code reçu pour {phone}  )
                try
            ...
            try
                ...
                if isinstance(entity, User)
                    logging.info(fCompte Telegram {phone} connecté au bot {bot_username})
                    await client.disconnect()
                    return True
            except Exception as e
                logging.error(fErreur lors de la connexion au bot {bot_username} {e})
                await client.disconnect()
                return False
        except Exception as e
            logging.error(fErreur lors de la validation du compte Telegram {phone} {e})
            await client.disconnect()
            return False
    ...
    def add_telegram_account(self, phone str, session_name str, api_id str, api_hash str)
        ...
        try
            ...
            logging.error(fErreur lors de l'ajout du compte {e})
            return False
    ...
    def remove_telegram_account(self, phone str)
        ...
        try
            ...
            logging.error(fErreur lors de la suppression du compte {e})
            return False
    ...
    def list_accounts(self)
        ...
        print(fn{Fore.CYAN}=== Comptes Telegram enregistrés ==={Style.RESET_ALL})
        for phone, data in self.accounts.items()
            print(fnNuméro  {phone})
            print(fSession  {data['session']})
            print(fAjouté le  {data['added_date']})
...
class SMMBot
    def __init__(self)
        ...
        self.tasks_clicked = False
        self.instagram_clicked = False
        self.load_accounts()

    def load_accounts(self)
        Charge le compte Telegram connecté et les comptes Instagram
        try
            instagram_accounts_file = 'configinstagram_accounts.json'
            if os.path.exists(instagram_accounts_file)
                with open(instagram_accounts_file, 'r') as f
                    self.instagram_accounts = json.load(f)
            else
                self.instagram_accounts = {}
            logging.info(fChargement réussi  {len(self.instagram_accounts)} comptes Instagram)
        except Exception as e
            logging.error(fErreur lors du chargement des comptes {e})
            self.instagram_accounts = {}
    ...
    async def verify_telegram_bot_connection(self, client)
        ...
        try
            ...
            if isinstance(entity, User)
                logging.info(fCompte Telegram connecté au bot {self.smm_bot_username})
                return True
            return False
        except Exception as e
            logging.error(fErreur lors de la vérification de la connexion au bot {e})
            return False

    async def simulate_human_interaction(self, client, message, button_text str = None)
        Simule une interaction humaine avec des délais lents
        try
            ...
            logging.error(fErreur lors de la simulation d'interaction {e})

    async def process_instagram_account(self, client, instagram_username)
        Traite un compte Instagram  sélectionne le compte et exécute la tâche si valide
        try
            # Réinitialiser le clic sur Instagram pour chaque nouveau compte
            self.instagram_clicked = False

            # Cliquer sur Tasks une seule fois
            if not self.tasks_clicked
                await client.send_message(self.smm_bot_username, 'start')
                await asyncio.sleep(self.human_behavior.get_typing_delay('start'))
                await asyncio.sleep(self.human_behavior.get_random_delay(2.0, 6.0))
                        ...
                        for row in message.buttons
                            for button in row
                                if 'Tasks' in button.text
                                    ...
                                    self.tasks_clicked = True
                                    break
                await asyncio.sleep(self.human_behavior.get_scroll_delay())

            # Cliquer sur Instagram une seule fois
            if not self.instagram_clicked
                async for message in client.iter_messages(self.smm_bot_username, limit=1)
                    if message.buttons
                        for row in message.buttons
                            for button in row
                                if 'Instagram' in button.text
                                    ...
                                    self.instagram_clicked = True
                                    break
                await asyncio.sleep(self.human_behavior.get_random_delay(3.0, 7.0))

            # Rechercher le nom d'utilisateur Instagram et cliquer une seule fois
            async for message in client.iter_messages(self.smm_bot_username, limit=1)
                if message.buttons
                    for row in message.buttons
                        for button in row
                            if instagram_username.lower() in button.text.lower()
                                ...
                                await asyncio.sleep(self.human_behavior.get_random_delay(2.0, 5.0))
                                await self.simulate_human_interaction(client, message, button.text)
                                logging.info(f✅ Compte Instagram {instagram_username} sélectionné)
                                
                                # Attendre le message suivant pour l'action
                                await asyncio.sleep(self.human_behavior.get_random_delay(3.0, 7.0))
                                async for action_message in client.iter_messages(self.smm_bot_username, limit=1)
                                    task = self.parse_task_message(action_message.text)
                                    if task and task['type'] in ['follow', 'like', 'comment']
                                        # Exécuter la tâche
                                        self.current_instagram_account = instagram_username
                                        if await self.execute_instagram_task(client, task, action_message)
                                            # Cliquer sur Complete une seule fois si présent
                                            if action_message.buttons
                                                for row in action_message.buttons
                                                    for button in row
                                                        if 'Complete' in button.text
                                                            await self.simulate_human_interaction(client, action_message, button.text)
                                                            logging.info(✅ Bouton 'Complete' cliqué)
                                                            break
                                            return True
                                    else
                                        logging.info(f❌ Aucune action valide pour {instagram_username}, passage au compte suivant)
                                        return False
                                return False
            logging.warning(f❌ Compte Instagram {instagram_username} non trouvé dans le menu)
            return False
        except Exception as e
            logging.error(f❌ Erreur lors du traitement du compte Instagram {instagram_username} {e})
            return False
    ...
    def get_next_instagram_account(self)
        ...
        if self.instagram_accounts_index = len(accounts)
            self.instagram_accounts_index = 0
            self.task_count += 1
            logging.info(fCycle de tâches terminé. Compteur {self.task_count})
        ...
        try
            ...
            async def handle_new_message(event)
                while True
                    ...
                    if await self.process_instagram_account(client, instagram_account)
                        logging.info(fTâche terminée pour {instagram_account})
                    else
                        logging.info(fPassage au compte Instagram suivant)
                    
                    # Pause longue pour éviter la détection
                    await asyncio.sleep(self.human_behavior.get_random_delay(10.0, 20.0))
            ...
            logging.error(fErreur lors du démarrage du bot {e})
            print(f{Fore.RED}✗ Erreur lors du démarrage du bot  {str(e)}{Style.RESET_ALL})

    async def execute_instagram_task(self, client, task, message)
        Exécute une tâche Instagram avec comportement humain lent
        try
            if not self.current_instagram_account
                logging.error(❌ Aucun compte Instagram actif)
                return False

            # Simuler une réflexion avant l'action
            await asyncio.sleep(self.human_behavior.get_random_delay(3.0, 7.0))
            
            # Connexion au compte Instagram
            instagram_client = InstagramClient()
            instagram_client.login(self.current_instagram_account, self.instagram_accounts[self.current_instagram_account]['password'])
            await asyncio.sleep(self.human_behavior.get_random_delay(3.0, 7.0))

            if task['type'] == 'follow'
                await asyncio.sleep(self.human_behavior.get_random_delay(2.0, 5.0))
                logging.info(f✅ Suivi du compte via lien  {task['link']})
                instagram_client.user_follow(task['target_id'])
                logging.info(f✅ Tâche de follow exécutée pour {task['target']})
            elif task['type'] == 'like'
                await asyncio.sleep(self.human_behavior.get_random_delay(2.0, 5.0))
                logging.info(f✅ Like du post via lien  {task['link']})
                instagram_client.media_like(task['media_id'])
                ...
                logging.info(f✅ Commentaire posté via lien  {task['link']} avec texte  {task['comment']})
                instagram_client.media_comment(task['media_id'], task['comment'])
    ...
    def parse_task_message(self, message str) - Dict
        Analyse le message pour extraire la tâche, le lien et le texte du commentaire
        try
            task = {}
            if 'follow' in message.lower()
                task['type'] = 'follow'
                link_match = re.search(r'(https[^s]+)', message)
                task['link'] = link_match.group(0) if link_match else ''
                username_match = re.search(r'@(w+)', message)
                task['target'] = username_match.group(1) if username_match else ''
                task['target_id'] = task['target']  # À remplacer par une vraie extraction d'ID
            elif 'like' in message.lower()
                task['type'] = 'like'
                link_match = re.search(r'(https[^s]+)', message)
                task['link'] = link_match.group(0) if link_match else ''
                task['target'] = task['link'].split('')[-2] if task['link'] else ''
                task['media_id'] = task['target']  # À remplacer par une vraie extraction d'ID
            elif 'comment' in message.lower()
                task['type'] = 'comment'
                link_match = re.search(r'(https[^s]+)', message)
                task['link'] = link_match.group(0) if link_match else ''
                comment_match = re.search(r'with text (.)(n$)', message)
                task['comment'] = comment_match.group(1) if comment_match else ''
                task['target'] = task['link'].split('')[-2] if task['link'] else ''
                task['media_id'] = task['target']  # À remplacer par une vraie extraction d'ID
            else
                return None
    ...
    async def setup_telegram_account(self)
        ...
        try
            ...
            logging.error(fErreur lors de la configuration du compte {e})
    ...
    def add_instagram_account(self, username str, password str)
        ...
        try
            ...
            logging.error(fErreur lors de l'ajout du compte Instagram {e})
    ...
    def remove_instagram_account(self, username str)
        ...
        try
            ...
            logging.error(fErreur lors de la suppression du compte Instagram {e})
            return False
    ...
    def list_instagram_accounts(self)
        ...
        print(fn{Fore.CYAN}=== Comptes Instagram enregistrés ==={Style.RESET_ALL})
        for username, data in self.instagram_accounts.items()
            print(fnNom d'utilisateur  {username})
            print(fAjouté le  {data['added_date']})
...
def main_menu()
    ...
    while True
        print(fn{Fore.CYAN}=== Menu Principal ==={Style.RESET_ALL})
        ...
        choice = input(nVotre choix  ).strip()
        ...
        if choice == 1
            ...
            print(fn{Fore.CYAN}=== Gestion des comptes Telegram ==={Style.RESET_ALL})
            print(1. Voir les comptes)
            print(2. Supprimer un compte)
            subchoice = input(nVotre choix  ).strip()
            ...
            print(fn{Fore.CYAN}=== Gestion des comptes Instagram ==={Style.RESET_ALL})
            ...
            if subchoice == 1
                while True
                    ...
                    choice = input(nVoulez-vous ajouter un autre compte Instagram  (on)  ).lower()
                    if choice != 'o'
                        break
            elif subchoice == 2
            ...
            print(fn{Fore.CYAN}=== Comptes enregistrés ==={Style.RESET_ALL})
            print(n=== Compte Telegram connecté ===)
            if bot.telegram_account
                print(fNuméro  {bot.telegram_account['phone']})
                print(fSession  {bot.telegram_account['session']})
            else
                print(f{Fore.YELLOW}Aucun compte Telegram connecté.{Style.RESET_ALL})
            print(n=== Comptes Instagram ===)
            bot.list_instagram_accounts()
        elif choice == 6
