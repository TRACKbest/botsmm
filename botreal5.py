...
class HumanBehavior:
    ...
    def get_typing_delay(text: str) -> float:
        ...
        return (words / 30) * 60 + random.uniform(1.0, 4.0)  # Vitesse de frappe réduite
    ...
    def get_reading_delay(text: str) -> float:
        ...
        words = len(text.split())
        return (words / 150) * 60 + random.uniform(2.0, 6.0)  # Vitesse de lecture réduite
...
class SMMBot:
    ...
    async def verify_instagram_account(self, client, instagram_username):
        """Vérifie si le compte Instagram est dans le menu du bot et exécute l'action"""
        try:
            # Envoyer /start avec un délai humain
            ...
            # Simuler le temps de chargement du menu
            await asyncio.sleep(self.human_behavior.get_random_delay(3.0, 7.0))
            
            # Rechercher le nom d'utilisateur Instagram dans le menu
            async for message in client.iter_messages(self.smm_bot_username, limit=1):
                if message.buttons:
                    for row in message.buttons:
                        for button in row:
                            if instagram_username.lower() in button.text.lower():
                                ...
                                await asyncio.sleep(self.human_behavior.get_random_delay(2.0, 5.0))
                                await self.simulate_human_interaction(client, message, button.text)
                                logging.info(f"✅ Compte Instagram {instagram_username} sélectionné dans le menu")
                                ...
                                async for action_message in client.iter_messages(self.smm_bot_username, limit=1):
                                    ...
                                    if task:
                                        await self.execute_instagram_task(task)
                                        return True
                                return True
                
                if instagram_username.lower() in message.text.lower():
                    await asyncio.sleep(self.human_behavior.get_reading_delay(message.text))
                    logging.info(f"✅ Compte Instagram {instagram_username} trouvé dans le message")
                    return True
    ...
    def parse_task_message(self, message: str) -> Dict:
        ...
        # Exemple de format attendu : "Please follow this account: [link] @username"
        # ou "Please like this post: [link]"
        # ou "Please comment on this post: [link] with text: [comment]"
        try:
            task = {}
            if 'follow' in message.lower():
                ...
                # Simulation : obtenir l'ID du compte (remplacer par une vraie implémentation)
                task['target_id'] = task['target']
            elif 'like' in message.lower():
                task['type'] = 'like'
                link_match = re.search(r'(https?://[^\s]+)', message)
                task['link'] = link_match.group(0) if link_match else ''
                task['target'] = task['link'].split('/')[-2] if task['link'] else ''
                # Simulation : obtenir l'ID du média (remplacer par une vraie implémentation)
                task['media_id'] = task['target']
